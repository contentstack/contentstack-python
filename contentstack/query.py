...
class Query:
    ...
    def content_type(self, content_type_id):
        """The unique ID of the content type of which
        you wish to retrieve the details. The uid is
        generated based on the title of the content
        type and it is unique across a stack
        Arguments:
            content_type_id {str} -- unique ID of the content type
            of which you wish to retrieve information
        Raises:
    ...
    def and_query(self, *query_objects):
        """Get entries that satisfy all the conditions
        provided in the '$and' query.
        Arguments:
            query_objects {object} -- query_objects for variable number
        ...
        ==============================
        [Example]:
        Let’s say you want to retrieve entries in which the Title
        field is set to 'Redmi Note 3' and the Color
        field is 'Gold'. The query to be used for such a case would be:
        The response will contain the entries where the values
        for Title is 'Redmi Note 3' and Color is 'Gold'
        For more details read document:
        # pylint: disable=line-too-long
        [https://www.contentstack.com/docs/developers/apis/content-delivery-api/#and-operator]
        # pylint: disable=line-too-long
            >>> from contentstack import Stack
            >>> stack = Stack
            >>> (
    ...
    def or_query(self, *query_objects):
        ...
        for more:
        [https://www.contentstack.com/docs/developers/apis/content-delivery-api/#or-operator]
            >>> from contentstack import Stack
            >>> stack = Stack
            >>> (
            >>>  api_key='stack_api_key',
    ...
    def less_than(self, key, value):
        ...
        lesser than the value provided in the condition.
        Arguments:
            key {str} -- The key to be constrained
        ...
        for more details read document:
        [https://www.contentstack.com/docs/developers/apis/content-delivery-api/#less-than]
            >>> from contentstack import Stack
            >>> stack = Stack
            >>> (
            >>>  api_key='stack_api_key',
    ...
    def less_than_or_equal_to(self, key, value):
        ...
        for more details read document:
        # pylint: disable=line-too-long
        [https://www.contentstack.com/docs/developers/apis/content-delivery-api/#less-than-or-equal-to]
        # pylint: disable=line-too-long
            >>> from contentstack import Stack
            >>> stack = Stack
            >>> (
    ...
    def greater_than(self, key, value):
        ...
        entry to be greater than the provided valu
        Arguments:
            key {str} -- The key to be constrained.
        ...
        Example:
        Let’s say you want to retrieve all the entries that have value of the
        Price in USD field set to a value that is greater than but not equal to 146.
        You can send the parameter as:
        for more details read document:
        [https://www.contentstack.com/docs/developers/apis/content-delivery-api/#greater-than]
            >>> from contentstack import Stack
            >>> stack = Stack
            >>> (
            >>>  api_key='stack_api_key',
    ...
    def greater_than_or_equal_to(self, key, value):
        ...
        key entry to be greater than or equal to the provided value.
        Arguments:
            key {str} -- The key to be constrained.
            value {object} -- value The value that provides an lower bound.
        Raises:
        ...
        Example:
        Let’s say you want to retrieve all the entries that have value of the
        Price in USD field set to a value that is greater than or equal to 146.
        You can send the parameter as:
        for more details read document
        # pylint: disable=line-too-long
        [https://www.contentstack.com/docs/developers/apis/content-delivery-api/#greater-than-or-equal-to]
        # pylint: disable=line-too-long
            >>> from contentstack import Stack
            >>> stack = Stack
            >>> (
    ...
    def not_equal_to(self, key, value):
        ...
        for more details read document:
        # pylint: disable=line-too-long
        [https://www.contentstack.com/docs/developers/apis/content-delivery-api/#not-equals-operator]
        # pylint: disable=line-too-long
            >>> from contentstack import Stack
            >>> stack = Stack
            >>> (
    ...
    def contained_in(self, key, *contained_in):
        ...
        Example:
        In the Product content type, you have a field named Price in USD. Now,
        you need to retrieve all the entries where value of this
        field is one among the given set of values.
        The query fired using the '$in' parameter is given below:
        # pylint: disable=line-too-long
        [https://www.contentstack.com/docs/developers/apis/content-delivery-api/#array-equals-operator]
        # pylint: disable=line-too-long
            >>> from contentstack import Stack
            >>> stack = Stack
            >>> (
    ...
    def not_contained_in(self, key, *not_contained_in):
        """Get all entries in which the value of a field does not match to
        any of the given values. This parameter will compare field values
        of entries to that of the values provided in the condition,
        and the query will retrieve entries that have field values
        that does not match to any of the values provided.
        Arguments:
            key {str} -- The key to be constrained.
        ...
        Example:
        In the Product content type, you have a field named Price in USD. Now,
        you need to retrieve the entries where the field value does not fall
        in the given set. You can send the parameter as:
        for more details read document:
        [https://www.contentstack.com/docs/developers/apis/
        content-delivery-api/#array-not-equals-operator]
            >>> from contentstack import Stack
            >>> stack = Stack
            >>> (
            >>>  api_key='stack_api_key',
    ...
    def exists(self, key):
        ...
        ==============================
        Example:
        In the Product content type, we have a field named Price in USD. Now,
        you want to retrieve all the entries in
    ...
    def include_reference(self, key):
        """When you fetch an entry of a content type that
        has a reference field, by default,the content of
        the referred entry is not fetched. It only fetches
        the UID of the referred entry,
        Arguments:
            key {str} -- The key to be constrained.
        Raises:
            ValueError: If key is None
        ...
        ==============================
        Example:
        If you wish to fetch the content of the entry that is
        included in the reference field, you need to use the
        include[] parameter, and specify the UID of the reference
        field as value. This informs Contentstack that the request
        also includes fetching the entry used in the specified
        reference field. for more: read API References:
    ...
    def ascending(self, key):
        ...
        ==============================
        Example:
        In the Product content type, if you wish to sort the
        entries with respect to their prices, the parameter can be used as:
            >>> from contentstack import Stack
            >>> stack = Stack
            >>> (
            >>>  api_key='stack_api_key',
    ...
    def descending(self, key):
        """When fetching entries, you can sort thems
        in the descending order with respect to the
    ...
    def only(self, *field_uid):
        """Specifies an list of field_uid keys in BASE
        object that would be included in the response.
        Arguments:
            *field_uid {object} -- comma seprated objects of the only
            reference keys to be included in response.
        Raises:
            ValueError: If field_uid is None
        Returns:
            [Query] -- Query object, so you can chain this call.
    ...
    def only_with_reference_uid(self, reference_field_uid, *field_uid):
        """Specifies an array of only keys that would be
        included in the response.
        Arguments:
            reference_field_uid {[str]} -- reference_field_uid Key
            who has reference to some other class object.
            *field_uid {object} -- comma separated uid
            objects which get excluded from the response.
        Raises:
        ...
        [Example:]
            ...
            >>> content_type = stack.content_type('content_type_uid')
            >>> query = content_type.query()
            >>> query = query.only_with_reference_uid
            >>> (
            >>> 'reference_field_uid',
            >>> 'field_uid 1',
            >>> 'field_uid 2'
        ...
        if isinstance(reference_field_uid, str):
            ...
            return self
        raise ValueError('reference_field_uid and field_uid both should be str')

    def except_with_reference_uid(self, reference_field_uid, *field_uid):
        """Specifies an array of except keys that would be
        ...
        [Example:]
            ...
            >>> content_type = stack.content_type('content_type_uid')
            >>> query = content_type.query()
            >>> query = query.except_with_reference_uid('price_in_usd',
            >>> 'field_uid 1', 'field_uid 2')
            >>> result = query.find()
        ==============================
        """
    ...
    def include_content_type(self):
        """Include Content Type of all returned
        objects along with objects themselves.
        Returns:
            [Query] -- Query object, so you can chain this call.
        ==============================
        [Example:]
    ...
    def regex(self, key, regex, modifiers=None):
        """
        Add a regular expression constraint for finding
        string values that match the provided
        regular expression. This may be slow for large data sets.
        Arguments:
            key {str} -- The key to be constrained.
            regex {str} -- The regular expression pattern to match
        Keyword Arguments:
            modifiers {str} -- Any of the following supported Regular
            expression modifiers (default: {None})
        ...
        ==============================
        """
        if isinstance(key, str) and isinstance(regex, str):
            ...
            self.__query_dict[key] = self.__query_value
            return self
        raise KeyError('Kindly provide a valid Key')
    ...
    def find(self):
        ...
        ==============================
        """
        if isinstance(self.__content_type_id, str) and len(self.__content_type_id) > 0:
    ...
    def __execute_query(self):
        ...
        result = self.__http_request.get_result\
            (
                self.__query_url,
                self.__query_params,
                self.__headers
